...
const { sessionService } = require('./prepare');
...
test('SessionServiceStartSession_SetCorrectCookie_ResponsePassed', async () => {
  ...
  expect(response.setHeader.mock.calls[0][1]).toEqual(
    `token=randomuuid; Expires=Fri, 01 Jan 2100 00:00:00 GMT; Path=/; Domain=www.example.com; SameSite=None; Secure; HttpOnly`
  );
});
...
test('SessionServiceEndSession_SetCorrectCookie_ResponsePassed', async () => {
  ...
  expect(response.setHeader.mock.calls[0][0]).toEqual('Set-Cookie');
  expect(response.setHeader.mock.calls[0][1]).toEqual(
    `token=deleted; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Path=/; Domain=www.example.com; SameSite=None; Secure; HttpOnly`
  );
});
