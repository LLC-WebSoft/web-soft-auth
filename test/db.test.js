...
test('DatabaseParseOperatorAndValue_ParseAllOperators_OperatorNotEqual', () => {
  ...
  for (const operator of operators) {
    results.push(database.parseOperatorAndValue(`${operator}value`));
  }
...
test('DatabaseGetPageClause_ReturnEmptyClause_ItemsOnPageNotANumber', () => {
  ...
  expect(database.getPageClause(null)).toEqual('');
});
...
test('DatabaseDelete_PassToQueryCorrectSql_AllArgsPassed', async () => {
  ...
  expect(database.pool.query.mock.calls[0][0]).toEqual('DELETE FROM "TestTable" WHERE "id" = $1 AND "username" = $2 RETURNING "field1"');
...
test('DatabaseUpdate_PassToQueryCorrectSql_AllArgsPassed', async () => {
  ...
  expect(database.pool.query.mock.calls[0][0]).toEqual('UPDATE "TestTable" SET "field3" = $1, "field2" = $2 WHERE "id" = $3 AND "username" = $4 RETURNING "field1"');
...
test('DatabaseSelect_PassToQueryCorrectSql_AllArgsPassed', async () => {
  ...
  expect(database.pool.query.mock.calls[0][0]).toEqual('SELECT "field1", "field2" FROM "TestTable" WHERE "id" = $1 AND "username" = $2 ORDER BY "field1" LIMIT 30 OFFSET 30');
  expect(database.pool.query.mock.calls[0][1]).toEqual([40, 'admin']);
});
...
test('DatabaseSelect_PassToQueryCorrectSqlWith*_NoFieldsPassed', async () => {
  ...
  expect(database.pool.query.mock.calls[0][0]).toEqual('SELECT * FROM "TestTable" WHERE "id" = $1 AND "username" = $2 ORDER BY "field1" LIMIT 30 OFFSET 30');
  expect(database.pool.query.mock.calls[0][1]).toEqual([40, 'admin']);
});

test('DatabaseInsert_PassToQueryCorrectSql_AllArgsPassed', async () => {
  await database.insert( 'TestTable', { id: 40, username: 'admin' }, ['field1'] );
  expect(database.pool.query.mock.calls[0][0]).toEqual('INSERT INTO "TestTable" ("id", "username") VALUES ($1, $2) RETURNING "field1"');
  expect(database.pool.query.mock.calls[0][1]).toEqual([40, 'admin']);
});
