...
class Logger {
  ...
  error(error) {
    this.logMessage('error', this.errorToString(error));
  }

  fatal(error) {
    this.logMessage('fatal', this.errorToString(error));
  }
  ...
  errorToString(error) {
    ...
    const internal = error.internal ? `Internal: ${error.internal};\n` : '';
    const cause = error.cause ? `nCause: ${error.cause.toString()};\n` : '';
    return `${stack}${internal}${cause}`;
  }
