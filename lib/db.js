...
class Database {
  ...
  async query(text, params) {
    try {
      logger.sql(text, params);
      const result = await this.pool.query(text, params);
      return result.rows;
    } catch (error) {
      if (error.code.startsWith(UNIQUE_ERROR_CODE_PREFIX)) {
        throw new ConnectionError({ ...ERRORS.DATA_CONFLICT, internal: error.message });
      }
      throw new ConnectionError({ ...ERRORS.DATA_ERROR, internal: error.message });
    }
  }
